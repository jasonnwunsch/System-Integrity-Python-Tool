            ...
            if os.path.exists(file_path):
                ...
                if file_path in saved_hashes:
                    if saved_hashes[file_path] != current_hash:
                        ...
                        send_notification(f"ALERT: {file_path} CHANGE DETECTED!")
...
def cleanup_temp_files(interval_seconds):
    while True:
        ...
        if deleted_files:
            ...
            send_notification(f"Temp file clear completed. {len(deleted_files)} files removed.")
        else:
...
# Desktop notifications with logging
